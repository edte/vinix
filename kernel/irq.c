...
// 加载 IDT 的函数
void load_idt(struct idt_entry *entries, int num_entries) {
    struct idt_descriptor desc = {.limit = (num_entries * sizeof(struct idt_entry)) - 1, .base = entries};

    // 将整个 IDT 表的地址及大小信息告诉 CPU
    lidt(&desc);
}
