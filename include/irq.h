...
// 中断栈帧结构体类型
typedef struct {
    uint64_t rip;    // 中断返回地址 (return instruction pointer)
    uint64_t cs;     // 中断返回代码段选择子 (code segment selector)
    uint64_t rflags; // RFLAGS 寄存器值
    uint64_t rsp;    // 中断返回堆栈指针 (stack pointer)
    uint64_t ss;     // 中断返回堆栈段选择子 (stack segment selector)
