...
// TODO: 把 kprintln 嵌套一层 kprintf 之后，%d 答应就会出现错误，始终是一个 100000 左右的数，应该是栈的问题？待解决
...
int kprintf(const char *fmt, ...) {
    unsigned int FRcolor = WHITE;
    unsigned int BKcolor = BLACK;
...
int kprintln(const char *fmt, ...) { return kprintf_color(WHITE, BLACK, fmt) + kprintf_color(WHITE, BLACK, "\n"); };
...
int kprintln_color(unsigned int FRcolor, unsigned int BKcolor, const char *fmt, ...) {
    return kprintf_color(FRcolor, BKcolor, fmt) + kprintf_color(FRcolor, BKcolor, "\n");
}
